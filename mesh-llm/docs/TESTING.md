...
## MLX distributed planning and smoke tests

### 24. Dry-run the deterministic MLX plan

Use this before touching real machines. It prints the selected cohort, deterministic bind ports, per-rank hostfile JSON, env, and QUIC shim routes.

Example:

```bash
just \
  --set model_name Qwen2.5-1.5B-Instruct-4bit \
  --set model_gb 3.2 \
  --set candidates 'host-a:48,host-b:48,host-c:24' \
  --set force_split 1 \
  mlx-plan
```
...
- JSON output with `selected_peer_ids`
- Per-rank `bind_addrs`, `hostfile_json`, and `shim_routes`

### 25. Two Macs over SSH, direct MLX ring

Use this to validate upstream MLX distributed inference before threading it into mesh orchestration. This test does not use the mesh data plane yet. It proves that two Macs can run one distributed `mlx_lm.server` job, expose HTTP on rank 0, and answer a real request.

Prereqs:

- Passwordless SSH from the control machine to both Macs
- `python3` available on both Macs
- `mlx` and `mlx-lm` installed in that Python environment on both Macs
- The model repo id or local model path works on both Macs
- The IPs passed via `--ip` are directly reachable by both Macs for the ring backend

Example:

```bash
just \
  --set ssh_host_a mac-a.local \
  --set ssh_host_b mac-b.local \
  --set ip_a 192.168.1.10 \
  --set ip_b 192.168.1.11 \
  --set model mlx-community/Qwen2.5-1.5B-Instruct-4bit \
  --set remote_python python3 \
  --set remote_http_port 18080 \
  --set starting_port 47000 \
  mlx-smoke
```

Expected result:

- Rank 0 becomes healthy on `http://<rank0-ip>:18080/health`
- `GET /v1/models` succeeds on rank 0
- `POST /v1/chat/completions` succeeds on rank 0
- Command exits with `✅ MLX smoke test passed`

Useful flags:

- `pipeline=1` to exercise MLX pipeline mode instead of tensor parallel mode
- `env=KEY=VALUE` to export one extra env var on both Macs
- `startup_timeout_secs=` / `request_timeout_secs=` for slow first loads

### 26. Two Macs over SSH, mesh-llm MLX distributed path

Use this to validate the real mesh-llm MLX path: deterministic cohort selection, QUIC-backed loopback shims, per-rank hostfiles, and rank-0 HTTP exposure.

Prereqs:

- Passwordless SSH from the control machine to both Macs
- `mesh-llm` binary available on both Macs
- `llama.cpp/build/bin` available on both Macs
- `mlx` and `mlx-lm` installed on both Macs
- The MLX model directory exists at the same path on both Macs
- The QUIC bind ports you choose are reachable between both Macs
...
just \
  ...
  --set ip_a 192.168.1.10 \
  --set ip_b 192.168.1.11 \
  --set model /Users/me/.models/Qwen2.5-1.5B-Instruct-4bit \
  --set remote_mesh_bin /Users/me/mesh-bundle/mesh-llm \
  --set remote_bin_dir /Users/me/mesh-bundle \
  --set remote_api_port 19337 \
  --set remote_bind_port_a 47010 \
  --set remote_bind_port_b 47011 \
  mlx-mesh-smoke
```

Expected result:

- Rank 0 responds on `http://<rank0-ip>:19337/v1/models`
- `POST /v1/chat/completions` succeeds through mesh-llm
- Command exits with `✅ MLX mesh smoke test passed`
